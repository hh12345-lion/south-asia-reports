...
export default function WhatIsPage() {
  return (
    <PageShell title="What Is an Africa Expert Witness?" subtitle="Independent expert evidence for UK legal proceedings involving African countries." breadcrumbs={[{ label: "Home", href: "/" }, { label: "What Is an Africa Expert Witness?" }]}>
      <p className="mb-4 text-[#453F4E] leading-relaxed">An Africa expert witness is an independent specialist instructed to provide expert evidence on country conditions, African law, or sector-specific matters in UK immigration tribunals, asylum appeals, and international arbitration. Unlike witnesses of fact, experts have an overriding duty to the court under CPR Part 35 to provide objective, impartial analysis.</p>
      <p className="mb-4 text-[#453F4E] leading-relaxed">In asylum and immigration cases, Africa experts produce country condition reports addressing whether a claimant faces a well-founded fear of persecution on return. Reports analyse state protection, internal relocation, and consistency with country guidance cases such as MOJ Somalia and MA Eritrea.</p>
      <p className="mb-4 text-[#453F4E] leading-relaxed">In investment treaty arbitration, Africa experts provide evidence on country conditions relevant to fair and equitable treatment and expropriation claims, regulatory frameworks, and African legal systems governing ICSID and UNCITRAL disputes.</p>
      <h2 className="mt-8 text-xl font-bold text-[#0D3B2E]">When to instruct an Africa expert</h2>
      <ul className="mt-4 list-disc space-y-2 pl-6 text-[#453F4E]">
...
export default function FeesPage() {
  return (
    <PageShell title="Africa Expert Witness Fees UK" breadcrumbs={[{ label: "Home", href: "/" }, { label: "Fees" }]}>
      ...
      <ul className="mt-4 space-y-2 text-[#453F4E]">
      ...
      <p className="mt-4 text-[#453F4E] leading-relaxed">AfricaExpertWitness works with Legal Aid funded cases. Prior authority from the Legal Aid Agency is required above standard thresholds. Our rates are compatible with LAA prior authority requirements. See our <a href="/guides/instructing-africa-experts-legal-aid" className="text-[#C8922A] hover:underline">Legal Aid instruction guide</a> for the prior authority process.</p>
      ...
      <p className="mt-4 text-sm text-[#453F4E]">Fees reflect market rates for qualified country conditions experts with PhD-level credentials and tribunal acceptance records. Final fees depend on complexity, urgency, and scope. Contact us for a quote tailored to your case.</p>
    </PageShell>
  );
}
""")
  ...
  return (
    <PageShell title="Instruct an Africa Expert Witness" subtitle="Confidential case submission - response within one business day." breadcrumbs={[{ label: "Home", href: "/" }, { label: "Contact" }]}>
      <motion className="grid gap-12 lg:grid-cols-3">
        ...
        <aside className="rounded-[12px] border border-[#D1E3D8] bg-[#F7F9F7] p-6 h-fit">
          <h2 className="font-bold text-[#0D3B2E]">Why instruct through us</h2>
          <ul className="mt-4 space-y-4 text-sm text-[#453F4E]">
...
export default function PrivacyPage() {
  return (
    <PageShell title="Privacy Policy" breadcrumbs={[{ label: "Home", href: "/" }, { label: "Privacy" }]}>
      <p className="text-[#453F4E] leading-relaxed">AfricaExpertWitness.com is operated to connect UK solicitors with qualified Africa expert witnesses. We collect personal data submitted through our contact form (name, organisation, email, phone, and case details) solely to respond to instruction requests and match appropriate experts.</p>
      <p className="mt-4 text-[#453F4E] leading-relaxed">Data is processed via Formspree and retained only as long as necessary to fulfil your request. We do not sell personal data. You may request deletion by emailing info@africaexpertwitness.com. We use cookies only where necessary for site functionality and analytics if enabled.</p>
    </PageShell>
  );
}
""")
...
export default function TermsPage() {
  return (
    <PageShell title="Terms of Use" breadcrumbs={[{ label: "Home", href: "/" }, { label: "Terms" }]}>
      <p className="text-[#453F4E] leading-relaxed">AfricaExpertWitness.com is an expert witness matching service. We are not a law firm and do not provide legal advice. Expert witnesses instructed through our service operate independently and owe their primary duty to the court under CPR Part 35.</p>
      <p className="mt-4 text-[#453F4E] leading-relaxed">Fees quoted are indicative and confirmed on instruction. Cancellation terms are agreed between the instructing solicitor and the expert at the point of engagement.</p>
    </PageShell>
  );
}
""")
    ...
    <>
      ...
      <PageShell title="Our Africa Expert Witnesses" subtitle="Qualified country specialists with tribunal and arbitration experience." breadcrumbs={[{ label: "Home", href: "/" }, { label: "Experts" }]}>
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {experts.map((expert) => (
            <article key={expert.name} className="rounded-[12px] border border-[#D1E3D8] bg-[#FFFDF9] p-6">
              <h2 className="text-lg font-bold text-[#0D3B2E]">{expert.name}</h2>
              <p className="text-sm font-medium text-[#C8922A]">{expert.jobTitle}</p>
              <p className="mt-3 text-sm text-[#453F4E] leading-relaxed">{expert.description}</p>
              <p className="mt-3 text-xs text-[#453F4E]"><strong>Expertise:</strong> {expert.expertise.join(", ")}</p>
            </article>
          ))}
        </div>
      </PageShell>
  ...
  return (
    <>
      ...
      <dl className="space-y-6">
        {filtered.map((t) => (
          <div key={t.term} id={t.term.toLowerCase().replace(/[^a-z0-9]+/g, "-")} className="rounded-[12px] border border-[#D1E3D8] bg-[#FFFDF9] p-5">
            <dt className="font-semibold text-[#0D3B2E]">{t.term}</dt>
            <dd className="mt-2 text-[#453F4E] leading-relaxed">
              {t.definition}
...
export default function QualificationsPage() {
  return (
    <PageShell title="Africa Expert Witness Qualifications & Vetting Standards" breadcrumbs={[{ label: "Home", href: "/" }, { label: "Qualifications" }]}>
      ...
      <p className="mt-4 text-[#453F4E]">PhD or research degree in African Studies, Political Science, International Relations, Anthropology, or Law. Field research in relevant African countries. Language skills including French, Swahili, Arabic, Amharic, Hausa, and Somali. Tribunal acceptance history and OSCOLA citation standard.</p>
      <h2 className="mt-8 text-xl font-bold text-[#0D3B2E]">African Law Experts</h2>
      <p className="mt-4 text-[#453F4E]">Qualified in relevant African jurisdiction(s). Knowledge of OHADA, ECOWAS, and SADC legal frameworks. Experience with BIT obligations. CPR Part 35 compliant.</p>
      <h2 className="mt-8 text-xl font-bold text-[#0D3B2E]">Investment Arbitration Experts</h2>
      <p className="mt-4 text-[#453F4E]">ICSID, UNCITRAL, ICC, and LCIA experience. Sector expertise in mining, energy, and infrastructure. African country and law knowledge.</p>
      <h2 className="mt-8 text-xl font-bold text-[#0D3B2E]">LGBTQI+ Specialists</h2>
      <p className="mt-4 text-[#453F4E]">Human rights advocacy and research background. Country-specific expertise. Training in sexuality and gender identity evidence standards.</p>
      <h2 className="mt-8 text-xl font-bold text-[#0D3B2E]">CPR Part 35</h2>
      <p className="mt-4 text-[#453F4E]">Experts owe an overriding duty to the court to provide independent, objective evidence. Reports must state material instructions, distinguish fact from opinion, and not omit material facts. Africa country evidence is subject to the same rigorous standards as any UK expert evidence.</p>
      <h2 className="mt-8 text-xl font-bold text-[#0D3B2E]">Legal Aid Compliance</h2>
      <p className="mt-4 text-[#453F4E]">Experts on our panel work within LAA prior authority frameworks. Standard country reports from £800 are compatible with typical prior authority thresholds. See our Legal Aid guide for the prior authority process.</p>
    </PageShell>
  );
}
""")
  ...
  return (
    <PageShell title="How to Instruct an Africa Expert Witness" subtitle="How AfricaExpertWitness matches specialists to your case." breadcrumbs={[{ label: "Home", href: "/" }, { label: "How to Instruct" }]}>
      <div className="space-y-6">
        {steps.map((s) => (
          <div key={s.n} className="flex gap-4 rounded-[12px] border border-[#D1E3D8] bg-[#FFFDF9] p-6">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#0D3B2E] font-bold text-white">{s.n}</span>
            <div><h2 className="font-bold text-[#0D3B2E]">{s.title}</h2><p className="mt-2 text-[#453F4E]">{s.body}</p></div>
          </div>
        ))}
      </div>
